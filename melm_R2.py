...
class melm:
    def main(self, TrainingData_File, TestingData_File, Elm_Type, NumberofHiddenNeurons, ActivationFunction, nSeed,
             verbose, Iteration, NeuronStack):
        ...
        if Elm_Type != self.REGRESSION:
            ...
            label = [sorted_target[0]]  # %   Find and save in 'label' class label from training and testing data sets
            j = 0
        ...
        if (ActivationFunction == 'erosion') or (ActivationFunction == 'dilation') or (
                ActivationFunction == 'fuzzy-erosion') or (ActivationFunction == 'fuzzy_erosion') or (
                ActivationFunction == 'fuzzy-dilation') or (ActivationFunction == 'fuzzy_dilation') or (
                ActivationFunction == 'bitwise-erosion') or (ActivationFunction == 'bitwise_erosion') or (
                ActivationFunction == 'bitwise-dilation') or (ActivationFunction == 'bitwise_dilation'):
            # InputWeight = np.random.uniform(np.amin(np.amin(P)), np.amax(np.amax(P)), (NumberofHiddenNeurons,
            # NumberofInputNeurons))
            if not NeuronStack:
                classAverage = np.average(classValues, axis=0)
                CounterClassAverage = np.average(counterClassValues, axis=0)
        ...
        TY = np.transpose(
            np.dot(np.transpose(tempH_test), OutputWeight))  # %   Y: the actual output of the training data
        ...
        if Elm_Type == self.CLASSIFIER:
            ...
            print('Training Accuracy: ' + str(TrainingAccuracy * 100) + ' % (',
                  str(np.size(label_index_expected, 0) - MissClassificationRate_Training), '/',
                  str(np.size(label_index_expected, 0)), ') (classification)')
            ...
            print('Testing Accuracy: ' + str(TestingAccuracy * 100) + ' % (',
                  str(np.size(label_index_expected, 0) - MissClassificationRate_Testing), '/',
                  str(np.size(label_index_expected, 0)), ') (classification)')
...
def setOpts(argv):
    ...
    return (arg.__dict__['TrainingData_File'], arg.__dict__['TestingData_File'], arg.__dict__['Elm_Type'],
            arg.__dict__['nHiddenNeurons'],
    ...
    for i in range(999):
        ...
        if i == 0:
            ...
            if (Dataset[i][1] == 100.0):
                ...
                Dataset.append(
                    ff.main(opts[0], opts[1], opts[2], 2 * (i + 1), opts[4], opts[5], opts[6], pd.DataFrame(), Weights))
                Values.append(Dataset[i][0])
                Accuracies.append(Dataset[i][1])
                break
